...
static size_t memArrSize[4]={60, 80, 50, 66};
...
const char* qu[2*4]; int front=-1, rear=-1;
...
unsigned short int isFull(void){return ((rear+1)%2*4==front);}
...
void enqueue(const char* processName)
{
    ...
    else
    {
        ...
        if(front==-1 && rear==-1) front=rear=0; else rear=(rear+1)%2*4; qu[rear]=copy;
    }
}
...
# 88 "Program_01.c"
    ...
    else
    {
        ...
        char* pName=(char*)qu[front]; if(front==rear && front!=-1) front=rear=-1; else front=(front+1)%2*4; return pName;
    }
}
...
size_t getSize(const char* processName)
{
    ...
    fseek(fp, 0, 
# 106 "Program_01.c" 3 4
                2
# 106 "Program_01.c"
                        ); size_t size=ftell(fp); rewind(fp); fclose(fp); return size;
}
...
int existsInMap(const char* processName){ void* out=
# 148 "Program_01.c" 3 4
                                                   ((void *)0)
# 148 "Program_01.c"
                                                       ; if(map_get(procMap, processName, strlen(processName)+1, &out))return 1; else return 0;}
...
void terminateProcess(int index)
{
    ...
    free(p); memArr[index].pr=
# 197 "Program_01.c" 3 4
                             ((void *)0)
# 197 "Program_01.c"
                                 ; memArr[index].free=1; printf("Partition %d is now free\n", index);
}
...
void checkWaitingQueue(size_t idx)
{
    ...
    const char* pname=isEmpty()?
# 205 "Program_01.c" 3 4
                               ((void *)0)
# 205 "Program_01.c"
                                   :qu[front];
...
int main(void)
{
    ...
    while(1)
    {
        ...
        printf(">> "); fgets(processName, sizeof(processName), 
# 273 "Program_01.c" 3 4
                                                              stdin
# 273 "Program_01.c"
